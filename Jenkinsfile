    ...
    stage('Deliver') {
       ...
       try {
        withCredentials([file(credentialsId: '1759702f-d204-4d3a-b9be-9c367665f7d4', variable: 'PEM_FILE')]) {
            sh 'ssh -i $PEM_FILE -o StrictHostKeyChecking=no ubuntu@ec2-54-251-168-251.ap-southeast-1.compute.amazonaws.com \'cd ~/production_build && rm -r *\''
            ...
            sh 'ssh -i $PEM_FILE ubuntu@ec2-54-251-168-251.ap-southeast-1.compute.amazonaws.com \'cd ~/production_build && npm install\''
            if (sh(script: 'echo $?', returnStatus: true) != 0) {
                error "Failed to start the server on the EC2 instance."
            }

            sh 'ssh -i $PEM_FILE ubuntu@ec2-54-251-168-251.ap-southeast-1.compute.amazonaws.com \'cd ~/production_build && npm run build\''
            if (sh(script: 'echo $?', returnStatus: true) != 0) {
                error "Failed to start the server on the EC2 instance."
            }

            sh 'ssh -i $PEM_FILE ubuntu@ec2-54-251-168-251.ap-southeast-1.compute.amazonaws.com \'cd ~/production_build && sudo pm2 serve 80 build --spa\''
            if (sh(script: 'echo $?', returnStatus: true) != 0) {
                error "Failed to start the server on the EC2 instance."
            }
